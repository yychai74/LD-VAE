...
def init_args():
    ...
    parser.add_argument("--model", default='LD_VAE', type=str)  # LS_PT
...
class Model(pl.LightningModule):
    def __init__(self, bert_learning_rate, other_learning_rate, adam_epsilon, warmup_steps, weight_decay,
        ...
        if args.model == "LD_VAE":
            self.model = LD_VAE(config=GPT2Config.from_pretrained("gpt"), preseqlen=10, mid_dim=512,
                                dataset=args.dataset,
                                tokenizer=self.tokenizer, use_adapter=False,
                                content_embeddings=train_dataset.content_embeddings)
        elif args.model == "LS_PT":
            self.model = LS_PT(config=GPT2Config.from_pretrained("gpt"), preseqlen=self.hparams.class_num * 10,
...
if __name__ == '__main__':
    ...
    gpt_tokenizer = GPT2Tokenizer.from_pretrained("gpt")
        ...
        for ckpt in all_checkpoints:
            ...
            with torch.no_grad():
                for p, total_num in prompts_counter.items():
                    ...
                    for idx in tqdm(range(2 * total_num // 10 + 1)):
                        ...
                        input_ids = tokenized["input_ids"].repeat(1, 1).to(device)
                        if args.model == "LD_VAE":
